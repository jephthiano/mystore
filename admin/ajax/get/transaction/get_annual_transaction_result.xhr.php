...
if(isset($_POST['st']) && isset($_POST['pg'])){
	...
	if($status2 === 'all'){
		...
		$board = 'all_time';//for dashboard
	}elseif(is_numeric($status2)){
		$add = "WHERE or_pmt_year = {$status2} AND or_payment_received = 'yes'";
		$board = 'per_year';//for dashboard
	}
	...
	if($status2 === 'all'){
		$sql = "SELECT DISTINCT or_pmt_year FROM order_table {$add} ORDER BY or_pmt_regdatetime DESC LIMIT $start,$display";
	}elseif(is_numeric($status2)){
		$sql = "SELECT DISTINCT or_pmt_month FROM order_table {$add} ORDER BY or_pmt_regdatetime DESC LIMIT $start,$display";
	}
	$stmt = $conn->prepare($sql);
	if($status2 === 'all'){$stmt->bindColumn('or_pmt_year',$year);}elseif(is_numeric($status2)){$stmt->bindColumn('or_pmt_month',$month);}
	$stmt->execute();
	...
	if(is_numeric($status2)){
		...
		<div class='j-container'>
			<a href="<?= file_location('admin_url','transaction/annual/'.process_year('previous',$status2).'/')?>">
			<span class='j-color1 j-text-color4 j-round j-btn j-left'>PREVIOUS</span>
			</a>
			<?php
			if(strtotime($status2) < strtotime(date("Y"))){
				?>
				<a href="<?= file_location('admin_url','transaction/annual/'.process_year('next',$status2).'/')?>">
			...
			<div class="j-responsive"style='line-height:27px;'>
				<table class="j-table-all j-border-0">
					<tr class="j-border-0 j-text-color1">
						<td><b>S/N</b></td>
						<td><b><?=$status2==='all'?'Year':'Month'?></b></td>
					...
					while($stmt->fetch()){
						if($status2 === 'all'){
							$amount = paid_sum('or_amount','or_pmt_year',$year);
							$delivery_fee = paid_sum('or_delivery_fee','or_pmt_year',$year);
							?>
							<tr class="j-border-0">
								<td><?php s_n();?></td><td><?=$year;?></td>
								...
								<td><a href='<?= file_location('admin_url','transaction/annual/'.$year.'/')?>'><i class="j-large <?= icon('eye');?>"></i></a></td>
							</tr>
							<?php
						}else{
