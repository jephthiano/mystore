...
if(isset($_POST['s']) && isset($_POST['st']) && isset($_POST['pg'])){
	...
	$status2 = ($_POST['st']);
	if($status2 === 'all'){
		$add= "or_payment_received = 'yes'";
	}elseif($status2 === 'pod'){
		$add= "or_payment_received = 'yes' AND or_payment_method = 'payment on delivery'";
	}elseif($status2 === 'card'){
		$add= "or_payment_received = 'yes' AND or_payment_method = 'card payment'" ;
	}
	...
	if(!empty($searchtext)){ // if the search text is not empty
		...
		WHERE (MATCH(or_order_id,or_token) AGAINST(:searchtext IN BOOLEAN MODE)) AND {$add}
		...
		WHERE {$add} ORDER BY or_pmt_regdatetime DESC LIMIT $start,$display";
		$stmt = $conn->prepare($sql);
		...
		<center>
			<div class="j-responsive"style='line-height:27px;'>
				<p class="j-text-color5">
					<b><?=empty($searchtext)?$numRow.' Transaction(s) found':$numRow." result(s) found for <span class='j-text-color1'> '".remove_last_value($searchtext)."' </span> in ".$status2." transaction";?></b>
				</p>
