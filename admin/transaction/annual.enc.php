...
$page_url = file_location('admin_url','transaction/annual/'.$_GET['status'].'/'.$_GET['page'].'/');
require_once(file_location('admin_inc_path','session_check.inc.php'));
if($adlevel < 3){trigger_error_manual(404);}
if(isset($_GET['status'])){
	$sta = ($_GET['status']);
	if(is_numeric($sta) && !empty($sta)){$status = $sta;}else{$status = 'all';}
}else{
$status = 'all';	
}
if(isset($_GET['page'])){
...
$page = "MANAGE ANNUAL TRANSACTIONS";
...
<div class="j-row">
	...
	<div class="j-col s12 l10"id='mainbody'>
		...
		<div id="maincol">
			<div class='j-padding'>
				<h2 class='j-text-color1 j-padding j-color4'><b><?=strtoupper($status)?> ANNUAL TRANSACTIONS LIST</b></h2>
			</div>
			<div id="shr"></div>
			<span id="st"></span>
		</div>
