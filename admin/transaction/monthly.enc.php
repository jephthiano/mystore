...
$page_url = file_location('admin_url','transaction/monthly/'.$_GET['status'].'/'.$_GET['page'].'/');
require_once(file_location('admin_inc_path','session_check.inc.php'));
if($adlevel < 3){trigger_error_manual(404);}
if(isset($_GET['status'])){
	$sta = ($_GET['status']);
	if(!empty($sta)){$status = $sta;}else{$status = 'all';}
}else{
$status = 'all';	
}
if(isset($_GET['page'])){
...
$page = "MANAGE MONTHLY TRANSACTIONS";
...
<div class="j-row">
	...
	<div class="j-col s12 l10"id='mainbody'>
		...
		<div id="maincol">
			<div class='j-padding'>
				<h2 class='j-text-color1 j-padding j-color4'><b><?=$status === 'all'?'ALL':strtoupper(show_date($status,'month'))?> MONTHLY TRANSACTIONS LIST</b></h2>
			</div>
			<div id="shr"></div>
			<span id="st"></span>
		</div>
